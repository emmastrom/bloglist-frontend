...
const Blog = ({ blog, updateBlog, currentUser, deleteBlog }) => {
  ...
  if (blogVisible === true) {
    return (
      <div className="blog-info">
        <p>{blog.title} {blog.author}<button className='clickView' onClick={() => setBlogVisible(false)}>hide</button></p>
        <a className='blog-url' href={blog.url}>{blog.url}</a>
        <p className='blog-likes'>{blog.likes} <button className='clickLikes' onClick={
          () => {
            try {
